<?php
    ...
    $query = "SELECT m.*, h.title FROM modal m left join home_detail h on m.home_id = h.id";
...
<?php foreach($data as $row): ?>
    <?php
        $texto = explode('<li>', $row[1]);
        $texto = implode('<li> <i class="fas fa-check"></i> ', $texto);
    ?>
    <div class="modal fade modales" id="home_<?= $row[2] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" >
