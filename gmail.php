<?php
  ...
  $gmail_general = $conection->conection_pdo()->query("SELECT * FROM gmail")->fetchAll(PDO::FETCH_OBJ);
  $gmail_detalles = $conection->conection_pdo()->query("SELECT * FROM gmail_detail ORDER BY orden ASC")->fetchAll(PDO::FETCH_OBJ);
?>
<!DOCTYPE html>
<html>
<head>
  ...
  <div class="pt-5">
    <nav style="--bs-breadcrumb-divider: '';" aria-label="breadcrumb p-5">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"><?= $gmail_general[0]->subtitle ? $gmail_general[0]->subtitle : 'Configuración Gmail' ?></li>
      </ol>
    </nav>
  </div>
</div>
<div class="container pt-5 pb-5">
  <div class="row align-items-start">
    <div class="col-md-9">
      <h2 class="text-center"><?= $gmail_general[0]->title ? $gmail_general[0]->title : 'Configuración Gmail' ?></h2>
      <?= $gmail_general[0]->description ? $gmail_general[0]->description : '' ?>
      <?php foreach($gmail_detalles as $key => $detalle): ?>
        <h5><?= ($key+1)?>) <?= $detalle->title ? $detalle->title : 'Titulo' ?></h5>
        <p class="pl-3 text-muted">
          <?= $detalle->description ? $detalle->description : '' ?>
        </p>
        <?php if(!empty($detalle->img)): ?>
          <img src="<?= $base_url ?>php/img/gmail/<?= $detalle->img ?>" class="rounded mx-auto d-block img-gmail" alt="...">
        <?php endif ?>
        <!-- <?= var_dump($detalle) ?> -->
      <?php endforeach ?>
    </div>
    <div class="col-md-3">
      <h4 class="text-center">Enlaces de su interes</h4>
      <ul class="list-group list-group-flush">
        <li class="list-group-item"><a href="<?= $base_url ?>support.php">Centro de ayudas</a></li>
      </ul>
    </div>
  </div>
</div>
